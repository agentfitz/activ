...
<article <?php post_class('dayd_post clearfix'); ?> id="article_<?php the_ID(); ?>">
	...
	<aside>
		...
		<!-- Picture -->
		<a title="<?php printf( esc_attr__('Permalink to %s', 'haku'), the_title_attribute('echo=0') ); ?>" href="<?php the_permalink(); ?>"><?php if ( has_post_thumbnail() ) the_post_thumbnail('post-thumbnail', array('title' => '') ); ?></a>
		...
		<?php if ( get_theme_option('post_tags') ) : ?>
	...
	<div>
		...
		<!-- Post title -->
		<h2 class="post_title">
			<a title="<?php printf( esc_attr__('Permanent link to %s', 'haku'), the_title_attribute('echo=0') ); ?>" href="<?php the_permalink(); ?>" rel="bookmark"><?php echo ( get_the_title() ? get_the_title() : get_the_time('F j') ); ?></a>
		</h2>
